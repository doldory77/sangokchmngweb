const Menu0101 = {
    ...
    <main class="container">
      ...
      <md-header :title="'우리교회는 ?'">
        <router-link class="btn btn-outline-light" :to="{name: 'Board', query: {kind:'MENU0101', bno:'', title:'우리교회는 ?'}}">등록</router-link>
      </md-header>
    
      <div class="my-3 p-3 bg-body rounded shadow-sm">
        <h6 class="d-none border-bottom pb-2 mb-0">Suggestions</h6>
        
        <bd-item v-for="item in boardItems" :key="item.bno" :kind="'MENU0101'" :bno="item.bno" :subject="item.subject" :content="item.content" :tagYn="item.tag_yn" :title="'우리교회는 ?'"></bd-item>
